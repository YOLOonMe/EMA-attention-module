...
# PP-LCNet
class EMA(nn.Module):
    def __init__(self, channels, c2, factor=32):
    ...
    def forward(self, x):
        ...
        weights = (torch.matmul(x11, x12) + torch.matmul(x21, x22)).sigmoid()
        return (group_x * weights.reshape(b * self.groups, 1, h, w)).reshape(b, c, h, w)
